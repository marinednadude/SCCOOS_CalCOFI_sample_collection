...
ggplot(data = world) +
    geom_sf() +
    geom_point(data = group_by_site_f_a8, 
               aes(x=Lon_deg_r, y=Lat_deg_r, colour=n), size=3, alpha=0.6) + 
  coord_sf(xlim = c(min_lon-5, max_lon+1), 
           ylim = c(min_lat-0.5, max_lat+2), 
           expand = FALSE) + 
  theme_bw() + 
  xlab("Longitude") +
  ylab("Latitude") +
...
```{r}
# Overlay MPAs
mpas <- st_read("ds582.shp")

socal <- mpas[mpas$Study_Regi %in% c('SCSR', 'CCSR'), ]

ana <- socal[socal$NAME == 'Anacapa Island SMR', ]

map <- basemap(data = coords, bathymetry=TRUE)

opc <- map + guides(fill = "none") +
  geom_sf(data = socal, color = "hotpink", alpha = 0.5) +
  coord_sf(xlim = c(min_lon+1.35, max_lon+1), 
           ylim = c(min_lat-0.5, max_lat+1), 
           expand = FALSE) +
  geom_point(data = group_by_site_f_a8, 
               aes(x=Lon_deg_r, y=Lat_deg_r, color=n), size=3, alpha=0.6) +
  scale_color_gradient(low = "yellow", high = "red", name="Number of Samples")
  
    scale_color_viridis_c(name="Number of Samples") 


+
  scale_color_viridis_d(name="Protected Area")  
...
coords <- data.frame(lon = c(min_lon-5, min_lon-5, max_lon+1, max_lon+1), lat = c(min_lat-0.5, min_lat-0.5, max_lat+2, max_lat+2))



transform_coord(dt), aes(x=lon, y=lat))
...
calcofi_tows_a8_pic %>% 
  filter(., year == 1999) %>% 
  arrange(Staline)
  filter(., site %in% c("93.3_30","90_60"))
```
