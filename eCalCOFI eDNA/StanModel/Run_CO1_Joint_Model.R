...
readRDS(here("data", "CO1_tech_nReads.RDS")) %>% 
  group_by(ID_CO1) %>% 
  filter(., CO1_reads >0) %>% 
  dplyr::summarise(occurrences = n_distinct(station_id), tot_reads = sum(CO1_reads)) %>% 
  arrange(desc(occurrences)) %>% 
  filter(., occurrences > 50 & tot_reads > 100000) %>% 
  filter(., str_detect(ID_CO1, " ")) -> seq_reads_to_keep

#restrict to taxa that aren't incredibly rare
common_count <- readRDS(here("data", "microscopy_tech_nReads.RDS")) %>%
  group_by(ID_count) %>%
  filter(., larval_counts >0) %>% 
  dplyr::summarise(tot = sum(larval_counts),occurrences=n_distinct(station_id)) %>%
  arrange(desc(tot)) %>% 
  slice(1:9) %>%
  pull(ID_count)
...
species_mapping <-
  ...
  filter( ID_count %in% common_count ) %>% #| ID_CO1 %in% seq_reads_to_keep$ID_CO1) %>%
  arrange(ID_main)
...
#data from zooplankton counts
dat.count <- readRDS(here("data", "microscopy_tech_nReads.RDS")) %>%
  filter(!ID_count %in% c("MISSING", "", "Disintegrated fish larvae")) %>%
  filter(., ID_count %in% species_mapping$ID_count ) %>%  # only include species in both datasets
 # filter(station_id %in% unique(dat.CO1$station_id)) %>%
  ...
  dplyr::summarise(larval_counts = ceiling(sum(larval_counts)))
...
dat.CO1 <- readRDS(here("data", "CO1_tech_nReads.RDS")) %>%
  ...
  filter(., ID_CO1 %in% species_mapping$ID_CO1 ) %>%  # only include species in both datasets
   dplyr::select(Sample,
                ID_CO1,
                CO1_reads,
                "station_id",
...
setdiff(dat.count$station_id, dat.CO1$station_id)

setdiff(dat.CO1$station_id,dat.count$station_id)
keepstations_tib = as_tibble(keepstations) %>% dplyr::rename(station_id=value)
# Compare
dplyr::cross_join(species_mapping, keepstations_tib) %>% 
  left_join(dat.count) %>% 
  left_join(dat.CO1) %>% 
  mutate(., Type = case_when(nReads > 0 & larval_counts > 0 ~ "Both Occur",
                             nReads > 0 & larval_counts == 0 ~ "eDNA Only",
                             nReads == 0 & larval_counts > 0 ~ "Larval Occur",
                             TRUE ~"Both Absent")) %>% 
  filter(., ID_main %in% common_count) %>% 
  group_by(ID_main, Type, station_id) %>% 
  ggplot(aes(x=ID_main, y=station_id, fill=Type)) + geom_tile() + theme(axis.text.x = element_text(angle = 90)) 

dplyr::cross_join(species_mapping, keepstations_tib) %>% 
  left_join(dat.count) %>% 
  left_join(dat.CO1) %>% 
  filter(., ID_main %in% common_count) %>% 
  ggplot(aes(x=log10(larval_counts), y=log10(nReads))) + geom_point() + facet_wrap(~ID_main, scales="free")
...
# dat.CO1 <- dat.CO1 %>%
#  filter(ID_CO1 %in% common_count)  #take just common species, for now

#species_mapping %>%
#  filter(ID_count %in% common_count) -> species_mapping
...
pos.CO1$station_id %>%  unique()
pos.count$station_id %>%  unique()
pos.all$station_id %>%  unique()

#require data from both of the datasets at a given station
keepstations2 <-
  pos.CO1$station_id %>% intersect(pos.count$station_id)

dat.count <- dat.count %>% filter(station_id %in% keepstations2)
dat.CO1 <- dat.CO1 %>% filter(station_id %in% keepstations2)
pos.all <- pos.all %>% filter(station_id %in% keepstations2)
pos.CO1 <- pos.CO1 %>% filter(station_id %in% keepstations2)
pos.count <- pos.count %>% filter(station_id %in% keepstations2)
...
stanMod = stan(
  ...
  init = stan_init_f2(
    n.chain = N_CHAIN,
    I_CO1 = I_CO1,
    N_station_rep_idx = stan_data$N_CO1_station_rep_idx
  )
)
...
# traceplot(stanMod, pars = c("a_CO1[1]", "a_CO1[2]", "a_CO1[3]"))
...
# For just Anchovy
po_2 <-
  ggplot(D_CO1 %>% filter(ID_CO1 == "Euphausia pacifica")) +
