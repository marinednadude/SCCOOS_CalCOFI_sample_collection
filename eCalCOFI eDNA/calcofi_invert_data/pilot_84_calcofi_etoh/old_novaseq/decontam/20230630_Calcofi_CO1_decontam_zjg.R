...
library(here)
source(here("new_miseq","decontam", "decontamination_utilities.R"))
...
input_meta_path <- here("old_novaseq", "sequence_mapping_file_20230714.csv")
...
dir.create(here("old_novaseq","decontam", "Output_csv"))
dir.create(here("old_novaseq","decontam", "Output_R"))
dir.create(here("old_novaseq","decontam", "Output_plots"))
...
saveRDS(hash.key, file = here("old_novaseq","decontam", "Output_R", "hash.key.RDS"))
...
saveRDS(ASV.table_long,
        file = here("old_novaseq","decontam", "Output_R", "ASV.table_long.RDS"))

saveRDS(ASV.table_used,
        file = here("old_novaseq","decontam", "Output_R", "ASV.table_used.RDS"))

hash.key <- readRDS(file = here("old_novaseq","decontam", "Output_R", "hash.key.RDS"))

ASV.table_used  <-
  readRDS(file = here("old_novaseq","decontam", "Output_R", "ASV.table_used.RDS"))
...
write.csv(pos.contam.species,
          file = here("old_novaseq","decontam", "Output_csv", "pos.contam.species.csv"))
...
write.csv(blank.contam.species,
          file = here("old_novaseq","decontam", "Output_csv", "blank.contam.species.csv"))
...
ggsave(
  plot = plot_1,
  here("old_novaseq","decontam", "Output_plots/Sample_Read_Depth_Barcode_1.png"),
  device = "png",
  width = 12,
  height = 8,
  units = "in"
...
ggsave(
  plot = vc_plot_1,
  here("old_novaseq","decontam", "Output_plots/Vector_Contamination_Barcode_1.png"),
  device = "png",
  width = 12,
  height = 8,
  units = "in"
...
all_distances.groups %>%
  saveRDS(file = here("old_novaseq","decontam", " all_distances.groups.rds"))
...
write_csv(
  to_write_discarded.step5 ,
  here("old_novaseq","decontam", "Output_csv", "step5.discared_samples.csv")
)
...
##Save Data -----
saveRDS(ASV.nested,
        file = here("old_novaseq","decontam", "Output_R", "ASV.nested_final.RDS"))
saveRDS(ASV.summary,
        file = here("old_novaseq","decontam", "Output_R", "ASV.summary_final.RDS"))
...
saveRDS(results_reads,file=here("old_novaseq","decontam","Output_R","results_sum.taxonomy_tech_reps_separate_read_counts.RDS"))
write_csv(results_reads ,here("old_novaseq","decontam","Output_csv","results_sum.taxonomy_tech_reps_separate_read_counts.csv"))
...
saveRDS(combined_index,file=here("old_novaseq","decontam","Output_R","results_sum.taxonomy_tech_reps_separate_eDNA_index.RDS"))
write_csv(combined_index ,here("old_novaseq","decontam","Output_R","results_sum.taxonomy_tech_reps_separate_eDNA_index.csv"))
