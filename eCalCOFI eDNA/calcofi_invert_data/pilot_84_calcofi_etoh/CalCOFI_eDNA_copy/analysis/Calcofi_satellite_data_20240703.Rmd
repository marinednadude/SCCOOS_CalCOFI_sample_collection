...
calcofi_sampling <-
  ...
  seq(as.Date("1995-04-01"), by = "month", length.out = 338) -> all_dates
  all_dates %>%  as_tibble() %>%
...
saveRDS(swsst1, file=here("data","sst_data_20240703.RDS"))

#Load
swsst1 <- readRDS(file = here("data", "sst_data_20240703.RDS"))
```
...
xpos <- c(-130 + 360, -116 + 360)
ypos <- c(29, 40)
time_save <-
seq(as.Date("1995-04-01"), by = "year", length.out = 29)
...
saveRDS(VIIRS, file = here("data/sst_region_data_20240703.RDS"))

#Load
VIIRS <- readRDS(file = here("data/sst_region_data_20240703.RDS"))
...
sst_avg_map <- melt_map(sanctchl1$longitude-360, sanctchl1$latitude, sanctchl1$avgmap)
   ...
   p = ggplot() + 
     ...
     aes(x = Longitude, y = Latitude, fill = avgmap), interpolate = TRUE, na.rm=T) +
         ...
         scale_fill_gradientn(colours = cmocean('thermal')(256), na.value = NA, breaks = c(10,12,14,16,18,20), limits = c(10,20), name = "Mean SST ˚C") +
     scale_bar(lon = -119.5, lat = 37.8, 
...
```
### Pre vs. 2014-2019 2020-2023
```{r}

xpos <- c(-130 + 360, -116 + 360)
ypos <- c(29, 40)
time_save <-
seq(as.Date("1995-04-01"), by = "year", length.out = 19)
tpos <- c(min(time_save), max(time_save))
sstInfo <- rerddap::info('erdPH53sstdmday', url = urlBase)
# 
 #Load data to save time
...
saveRDS(pre_2014, file = here("data/pre_2014_sst_region_data_20240703.RDS"))
...
saveRDS(post_2014, file = here("data/post_2014_sst_region_data_20240703.RDS"))
...
ypos <- c(29, 40)
time_save <-
seq(as.Date("2020-04-01"), by = "year", length.out = 4)
tpos <- c(min(time_save), max(time_save))
sstInfo <- rerddap::info('erdPH53sstdmday', url = urlBase)
# 
#Load data to save time
...
saveRDS(post_2019, file = here("data/post_2019_sst_region_data_20240703.RDS"))

#Load
post_2019 <- readRDS(file = here("data/post_2019_sst_region_data.RDS"))
...
pre_sst_avg_map <- melt_map(pre_2014$longitude-360, pre_2014$latitude, pre_2014$avgmap)
   ...
   pre_map = ggplot() + 
     ...
     aes(x = Longitude, y = Latitude, fill = avgmap), interpolate = TRUE, na.rm=T) +
         ...
         theme_bw(base_size = 12) + ylab("Latitude") + xlab("Longitude") +
         coord_fixed(1.3, xlim = xpos, ylim = ypos) +
         scale_fill_gradientn(colours = cmocean('thermal')(256), na.value = NA,breaks=b, labels=format(b), name = "Mean SST ˚C", limits = c(12,20))   +ggtitle(label = "1996-2013")
   pre_map
...
post_sst_avg_map <- melt_map(post_2014$longitude-360, post_2014$latitude, post_2014$avgmap)
   ...
   post_map = ggplot() + 
     ...
     aes(x = Longitude, y = Latitude, fill = avgmap), interpolate = TRUE, na.rm=T) +
         ...
         theme_bw(base_size = 12) + ylab("Latitude") + xlab("Longitude") +
         coord_fixed(1.3, xlim = xpos, ylim = ypos) +
         scale_fill_gradientn(colours = cmocean('thermal')(256), na.value = NA, breaks=b, labels=format(b),, name = "SST ˚C", limits = c(12,20))  +ggtitle(label = "2014-2019") +theme(legend.position = "none")
   post_map
...
xpos <- c(-125, -117)
ypos <- c(31.5, 36.5)
post_2019$avgmap <- apply(post_2019$sea_surface_temperature,c(1,2),function(x) mean(x,na.rm=TRUE))

post_2019_sst_avg_map <- melt_map(post_2019$longitude-360, post_2019$latitude, post_2019$avgmap)

   post_2019_map = ggplot() + 
               geom_raster(data = post_2019_sst_avg_map, 
     aes(x = Longitude, y = Latitude, fill = avgmap), interpolate = TRUE, na.rm=T) +
         geom_polygon(data = coast, aes(x=long, y = lat, group = group), fill = "grey80") +
         theme_bw(base_size = 12) + ylab("Latitude") + xlab("Longitude") +
         coord_fixed(1.3, xlim = xpos, ylim = ypos) +
         scale_fill_gradientn(colours = cmocean('thermal')(256), na.value = NA, breaks=b, labels=format(b),, name = "SST ˚C", limits = c(12,20)) +ggtitle(label = "2020-2023") +theme(legend.position = "none")
   post_2019_map
...
```{r}
library(patchwork)
pre_map + post_map + post_2019_map + plot_layout(guides = 'collect')+ plot_annotation(tag_levels = 'A')

ggsave(
  file = here::here("analysis", "figures", "Figure_1_OK_proposal.png"),
  width = 12,
  height = 8
)
