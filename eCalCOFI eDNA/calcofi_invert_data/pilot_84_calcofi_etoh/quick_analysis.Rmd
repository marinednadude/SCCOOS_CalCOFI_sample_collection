...
```{r}
#Load Data
ASV.summary <- readRDS( file = here("decontam", "Output_R", "ASV.summary_final.RDS"))

hash.key <- readRDS(file= here("decontam","Output_R","hash.key.RDS"))

#edna Data
all_samples <- readRDS(file=here("old_novaseq","decontam","Output_R","results_sum.taxonomy_tech_reps_separate_eDNA_index.RDS")) #this is eDNA index
colnames(all_samples)
all_samples_reads <- readRDS(file=here("decontam","Output_R","results_sum.taxonomy_tech_reps_separate_read_counts.RDS"))
...
method.adonis_results <- adonis2(method.rel_abun~Station+Year, data=method.sampledf)
...
##Plot_Ordination
plot_ordination(physeq_obj.all_eindex, ord, "samples", color = "Year", shape="Station") +
  ...
  labs(shape = "Station", color = "Year") -> figure_1
